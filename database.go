...
func (d *Database) FetchRecentListenDataByTime(playedAts []interface{}, recentListenedToIDs []interface{}) ([]models.RecentListenData, error) {
	...
	sql := fmt.Sprintf("SELECT %s FROM %s WHERE recent_listen_id IN (%s) AND played_at IN (%s)", columnNames, tableName, PrepareInStringPG(1, len(recentListenedToIDs), 1), PrepareInStringPG(1, len(playedAts), len(recentListenedToIDs)))
	vars := []interface{}{}
